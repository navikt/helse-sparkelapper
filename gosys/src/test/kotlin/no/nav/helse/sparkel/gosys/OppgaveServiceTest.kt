...
class OppgaveServiceTest {
    ...
    @Test
    fun `teller ikke med oppgaver med oppgavetype vurder notat`() {
        val forventetAntall = 0
        val oppgaverSomIkkeSkalTellesMed =
            listOf(
                objectNode().run {
                    put("oppgavetype", "VURD_NOTAT")
                },
            ).leggPåOpprettetTidspunkter()
        val oppgavehenter =
            Oppgavehenter { _, _ ->
    ...
    @Test
    fun `teller ikke med oppgaver med oppgavetype retur`() {
        val forventetAntall = 0
        val oppgaverSomIkkeSkalTellesMed =
            listOf(
                objectNode().run {
                    put("oppgavetype", "RETUR")
                },
            ).leggPåOpprettetTidspunkter()
        val oppgavehenter =
            Oppgavehenter { _, _ ->
...
private fun List<ObjectNode>.leggPåOpprettetTidspunkter() = map { it.put("opprettetTidspunkt", OffsetDateTime.now().format(DateTimeFormatter.ISO_ZONED_DATE_TIME)) }
