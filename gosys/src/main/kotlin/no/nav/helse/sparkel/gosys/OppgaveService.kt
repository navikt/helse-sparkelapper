...
internal class OppgaveService(
    private val oppgavehenter: Oppgavehenter,
) {
    ...
    private fun JsonNode.harIgnorerbarGjelderverdi() =
        GjelderverdierSomIkkeSkalTriggeVarsel.inneholder(
            finnVerdi("behandlingstype"),
            finnVerdi("behandlingstema"),
        )

    private fun JsonNode.harIgnorerbarOppgavetype() =
        OppgavetypeSomIkkeSkalTriggeVarsel.inneholder(
            finnVerdi("oppgavetype"),
        )

    private fun JsonNode.erFeilutbetalingsoppgaveSomErForGammel(ikkeEldreEnn: LocalDate) =
        opprettetTidspunkt().isBefore(ikkeEldreEnn) &&
            GjelderverdierSomIkkeSkalTriggeVarselHvisOppgavenOverEtÅrGammel.inneholder(
                finnVerdi("behandlingstype"),
                finnVerdi("behandlingstema"),
            )
