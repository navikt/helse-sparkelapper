...
val mockGenerator = mockk<ResponseGenerator>(relaxed = true) {
    every { organisasjon() }.returns(organisasjonResponse())
}

fun eregMockClient(mockResponseGenerator: ResponseGenerator) = HttpClient(MockEngine) {
    engine {
        addHandler { request ->
            when {
                request.url.fullPath.startsWith("/v1/organisasjon") -> {
                    respond(mockResponseGenerator.organisasjon())
                }
...
interface ResponseGenerator {
    fun organisasjon() = organisasjonResponse()
}

@Language("JSON")
fun organisasjonResponse() = """{
    "navn": {
        "bruksperiode": {
