    ...
    internal suspend inline fun <reified T> hentFraAareg(
        ...
        retry("arbeidsforhold") {
            ...
            try {
                ...
                throw if (response.status == HttpStatusCode.NotFound && melding == "Ukjent ident") {
                    UkjentIdentException()
                } else {
                    AaregException(melding, responseBody, response.status, e)
                }
            }
        }

    private fun feilmeldingFraAaregEllerGenerellTekst(responseBody: String) =
        try {
...
internal fun JsonNode.asOptionalLocalDate() = takeIf(JsonNode::isString)?.asString()?.takeIf(String::isNotEmpty)?.let { LocalDate.parse(it.substring(0, 10)) }

internal fun JsonNode.asLocalDate() = asString().substring(0, 10).let { LocalDate.parse(it) }
