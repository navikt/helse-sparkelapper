...
internal sealed class ForespurtOpplysning {
    companion object {
        fun List<ForespurtOpplysning>.toJsonMap() =
            map { forespurtOpplysning ->
...
internal fun JsonNode.asBestemmendeFraværsdager() = associate { it["organisasjonsnummer"].asString() to it["førsteFraværsdag"].asLocalDate() }

internal fun JsonNode.asForespurteOpplysninger(): List<ForespurtOpplysning> = mapNotNull(JsonNode::asForespurtOpplysning)

internal fun JsonNode.asForespurtOpplysning() =
    when (val opplysningstype = this["opplysningstype"].asString()) {
        "Inntekt" -> Inntekt
        "Refusjon" -> Refusjon
