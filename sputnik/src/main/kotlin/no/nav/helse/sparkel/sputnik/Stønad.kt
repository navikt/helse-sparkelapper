...
internal class Stønad private constructor(
    ...
    internal fun leggTilLøsning(
        packet: JsonMessage,
        stønadsperioder: Set<Stønadsperiode>,
    ) {
        val (fom, tom) = periode(packet)
        val aktuelleStønadsperioder =
            stønadsperioder
    ...
    internal companion object {
        ...
        private fun Set<Stønadsperiode>.barnSykdomLøsning() =
            map {
                mapOf(
                    "fom" to "${it.fom}",
                    "tom" to "${it.tom}",
        ...
        internal fun Iterable<Stønad>.omsluttendePeriode(packet: JsonMessage) =
            map { it.periode(packet) }.let { allePerioder ->
