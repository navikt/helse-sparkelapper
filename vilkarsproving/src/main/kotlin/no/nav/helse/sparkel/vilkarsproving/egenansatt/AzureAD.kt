...
class AzureAD(val props: AzureADProps) {
    ...
    private fun fetchToken(): Token {
        val (responseCode, responseBody) = with(props.tokenEndpointURL.openConnection() as HttpURLConnection) {
            ...
            outputStream.use { os ->
                os.writer().write(URLEncoder.encode("client_id=${props.clientId}&client_secret=${props.clientSecret}&scope=api://${props.nomAppName}/.default&grant_type=client_credentials", "utf-8"))
            }
