...
class MeldekortUtbetalingsgrunnlagClientTest {
    ...
    fun `mapper tom respons fra arena`() {
        @Language("XML")
        val response = """<ns2:finnMeldekortUtbetalingsgrunnlagListeResponse xmlns:ns2="http://nav.no/tjeneste/virksomhet/meldekortUtbetalingsgrunnlag/v1">
    <response/>
</ns2:finnMeldekortUtbetalingsgrunnlagListeResponse>"""

        val (httpClient, meldekortClient) = mockClient(xmlResponse(response))
