...
private fun Infotrygdperiode.overlappPåAnnenArbeidsgiverMedGrad100(other: Infotrygdperiode) =
    this.organisasjonsnummer != null &&
        other.organisasjonsnummer != null &&
        // Kun aktuelt å sammenligne når vi har orgnr på begge
        this.organisasjonsnummer != other.organisasjonsnummer &&
        // Kun aktuelt når periodene er på forskjellige arbeidsgivere
        this.grad == 100 &&
        other.grad == 100 &&
        // Kun aktuelt når begge periodene har 100%
...
internal fun List<Infotrygdperiode>.markerUsikkerGrad(): List<Infotrygdperiode> {
    ...
    return sortert.mapIndexed { index, denne ->
        ...
        if (resten.any { it.overlappPåAnnenArbeidsgiverMedGrad100(denne) }) {
            InfotrygdperiodeMedUsikkerGrad(denne)
        } else {
            InfotrygdperiodeMedSikkerGrad(denne)
        }
    }
}
