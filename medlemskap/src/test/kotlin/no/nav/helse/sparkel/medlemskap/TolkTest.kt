...
class TolkTest {
    ...
    @Test
    fun `vanlig response`() {
        assertEquals("JA", nyTolk(response = vurderingResponse("JA")).tolk())
        assertEquals("NEI", nyTolk(response = vurderingResponse("NEI")).tolk())
        assertEquals("UAVKLART", nyTolk(response = vurderingResponse("UAVKLART")).tolk())
        assertEquals("UAVKLART_MED_BRUKERSPORSMAAL", nyTolk(response = vurderingResponse("UAVKLART_MED_BRUKERSPORSMAAL")).tolk())
    }

    @Test
    fun `speil response`() {
        assertEquals("JA", nyTolk(response = speilvurderingResponse("JA")).tolk())
        assertEquals("NEI", nyTolk(response = speilvurderingResponse("NEI")).tolk())
        assertEquals("UAVKLART", nyTolk(response = speilvurderingResponse("UAVKLART")).tolk())
        assertEquals("UAVKLART_MED_BRUKERSPORSMAAL", nyTolk(response = speilvurderingResponse("UAVKLART_MED_BRUKERSPORSMAAL")).tolk())
    }

    @Test
    fun `graderte personer`() {
        assertEquals("UAVKLART", nyTolk(httpStatus = 503, response = "HVA_SOM_HELST_som_inneholder_GradertAdresseBørGiDette").tolk())
    }

    @Test
    fun `uventet response`() {
        assertEquals("Fikk uventet svar fra medlemskapstjenesten 3 ganger. Se sikkerlogg for detaljer", assertThrows<IllegalStateException> {
            nyTolk(response = crazyRespone).tolk()
        }.message)
    }

    @Test
    fun `ingen response`() {
        assertEquals("Fikk ikke kontakt med medlemskapstjenesten på 3 forsøk. Se sikkerlogg for detaljer", assertThrows<IllegalStateException> {
            exceptionTolk().tolk()
        }.message)
    }

    private fun nyTolk(httpStatus: Int = 200, response: String) = Tolk("11111111111", LocalDate.EPOCH, LocalDate.EPOCH) {
        httpStatus to response
    }

    private fun exceptionTolk() = Tolk("11111111111", LocalDate.EPOCH, LocalDate.EPOCH) {
        throw IOException("Noe nettverkskrøll")
    }
    ...
    @Language("JSON")
    private fun speilvurderingResponse(svar: String) = """
    {
...
""".trimIndent()
    ...
    @Language("JSON")
    private fun vurderingResponse(svar: String) = """
    {
        "kanal": "kafka",
        "resultat": {
            "svar": "$svar",
